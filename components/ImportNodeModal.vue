...
const { handleSubmit, handleReset } = useForm({
  ...
  initialValues: {
    importNodes: [{ tag: '', link: '' }]
  }
})

const { remove, push, fields } =
  useFieldArray<z.infer<typeof schema>['importNodes'][number]>('importNodes')
    ...
    <div class="space-y-4">
      <div class="flex flex-col gap-2">
        <InputGroup v-for="(field, index) in fields" :key="field.key">
          ...
          <Field #="{ componentField }" :name="`importNodes.${index}.link`">
            <InputText placeholder="link" v-bind="componentField" />
          </Field>

          <Field #="{ componentField }" :name="`importNodes.${index}.tag`">
